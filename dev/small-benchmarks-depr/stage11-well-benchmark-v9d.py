...
# -*- coding: utf-8 -*-
"""
Stage 11 — Well Benchmark v9d
-----------------------------
This version *uses* the fitted funnel in the parser ("physics") via two priors:
  - Depth prior φ(r): deeper toward the center (from the data-fit radial profile).
  - Slope prior g(r): steeper walls near the throat (from -dz/dr of the profile).

Coupling (safe by default):
  1) Slope-weighted energy:   S <- S * (1 + beta_s * g(r)^q_s)
  2) Zero-mean depth prior:   after MF, score' = score * (1 + alpha * zscore(φ)[peak])
  3) Selection requires BOTH relative and absolute thresholds:
       - relative: score' ≥ tau_rel * max_score'
       - absolute: score' ≥ tau_abs (95th pct of null via circular shifts)

Viz: data-fit radial funnel (like v9c3) with finite-core template blend. The surface matches the cloud.

CSV/JSON schema unchanged; JSON gains `geometry_v9d` with fitted-profile params.

python3 stage11-well-benchmark-v9d.py \
  ...
  --fit_quantile 0.6 --rbf_bw 0.25 \
  --core_k 0.18 --core_p 1.7 --core_r0_frac 0.14 \
  --blend_core 0.25 --template_D 1.2 --template_p 1.6 \
  --alpha 0.08 --beta_s 0.25 --q_s 2 \
  --tau_rel 0.6 --tau_abs_q 0.95 --null_K 40
...
def geodesic_parse_v9d(traces, priors, *, sigma=9, proto_width=160,
                        alpha=0.08, beta_s=0.25, q_s=2,
                        tau_rel=0.6, tau_abs_q=0.95, null_K=40, seed=0):
    ...
    # zero-mean depth prior at winner index
    phi_z = (phi_t - phi_t.mean()) / (phi_t.std() + 1e-9)
    score_resc = {p: max(0.0, score[p] * (1.0 + alpha * phi_z[peak[p]])) for p in keys}

    # thresholds
    smax = max(score_resc.values()) + 1e-12
    rng = _rng(int(seed) + 1337)
...
def main():
    ap = argparse.ArgumentParser(description="Stage 11 — v9d: fitted-prior coupling + data-fit funnel viz")
    # data
    ...
    ap.add_argument("--fit_quantile", type=float, default=0.60)
    ap.add_argument("--rbf_bw", type=float, default=0.25)
    ...
    # parser coupling (baked safe defaults)
    ap.add_argument("--alpha", type=float, default=0.08, help="zero-mean depth prior scale")
    ap.add_argument("--beta_s", type=float, default=0.25, help="slope weighting gain")
    ap.add_argument("--q_s", type=int, default=2, help="slope weighting exponent")
    ap.add_argument("--tau_rel", type=float, default=0.60, help="relative keep threshold")
    ap.add_argument("--tau_abs_q", type=float, default=0.95, help="null quantile for abs threshold")
    ap.add_argument("--null_K", type=int, default=40, help="null shifts for abs threshold")
    args = ap.parse_args()

    # ----- Build GLOBAL fitted profile first (priors + viz) -----
    ...
    for i in range(1, args.samples+1):
        ...
        keep_g, order_g = geodesic_parse_v9d(
            traces, priors, sigma=args.sigma, proto_width=160,
    ...
    out_fit = args.out_plot.replace(".png", "_v9d_fit.png")
    plot_surface(Xs, Ys, Zs, X3_raw, E, "Stage 11 — Data-fit Funnel (v9d, used as prior)", out_fit)
    ...
    summary = dict(
        ...
        plot_fitted=out_fit,
        geometry_v9d=dict(
            params=dict(alpha=args.alpha, beta_s=args.beta_s, q_s=args.q_s,
