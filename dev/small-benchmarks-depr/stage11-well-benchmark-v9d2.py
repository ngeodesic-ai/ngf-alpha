...
# -*- coding: utf-8 -*-
"""
Stage 11 — Well Benchmark v9d2 (fixed)
--------------------------------------
Recall-friendly coupling of a *data-fit, finite-core* funnel into the parser.
This version includes the missing `stock_parse` baseline.

Coupling:
  - Slope prior g(r): slope-weighted energy with unit-mean normalization.
  - Depth prior φ(r): zero-mean then ReLU at peak (non-negative bonus only).
  - Dual thresholds: relative + absolute from a circular-shift null.

Viz:
  - 3D PCA cloud with a 360° radial surface that fits the cloud (median
    by radius), plus a finite-core template blend for logo-like shape.

python3 stage11-well-benchmark-v9d2.py \
  ...
  --fit_quantile 0.65 --rbf_bw 0.30 \
  --alpha 0.05 --beta_s 0.25 --q_s 2 \
  --tau_rel 0.60 --tau_abs_q 0.92 --null_K 30
...
def fit_radial_profile(X3: np.ndarray, center: np.ndarray, r_grid: np.ndarray,
    ...
    # enforce monotone decrease toward center
    last = z_fit[-1]
...
def radius_from_sample_energy(S: Dict[str,np.ndarray]) -> np.ndarray:
    ...
    M = np.stack([_z(S[p]) for p in PRIMS], axis=1)
...
def null_threshold(signal: np.ndarray, proto: np.ndarray, rng, K=30, q=0.90):
    n = len(signal)
    vals = []
    for _ in range(K):
...
def geodesic_parse_v9d2(traces, priors, *, sigma=9, proto_width=160,
                        alpha=0.06, beta_s=0.20, q_s=2,
                        tau_rel=0.55, tau_abs_q=0.90, null_K=30, seed=0):
    keys = list(traces.keys())
    ...
    # depth prior: zero-mean then ReLU (non-negative)
    phi_z = (phi_t - phi_t.mean()) / (phi_t.std() + 1e-9)
    phi_pos = np.maximum(0.0, phi_z)
    score_resc = {p: max(0.0, score[p] * (1.0 + alpha * phi_pos[peak[p]])) for p in keys}

    # thresholds (relative + absolute from null on the already-weighted signal)
    smax = max(score_resc.values()) + 1e-12
    rng = _rng(int(seed) + 7331)
...
def main():
    ap = argparse.ArgumentParser(description="Stage 11 — v9d2 (fixed): data-fit funnel prior + recall-friendly parser")
    # data
    ...
    ap.add_argument("--beta_s", type=float, default=0.20)
    ap.add_argument("--q_s", type=int, default=2)
    ap.add_argument("--tau_rel", type=float, default=0.55)
    ap.add_argument("--tau_abs_q", type=float, default=0.90)
    ap.add_argument("--null_K", type=int, default=30)
    args = ap.parse_args()

    # ----- Build global fitted profile for priors + viz -----
    ...
    for i in range(1, args.samples+1):
        ...
        keep_g, order_g = geodesic_parse_v9d2(
    ...
    out_fit = args.out_plot.replace(".png", "_v9d2_fit.png")
    plot_surface(Xs, Ys, Zs, X3_raw, E, "Stage 11 — Data-fit Funnel (v9d2, prior in use)", out_fit)
    ...
    summary = dict(
        ...
        plot_raw=args.out_plot, plot_fitted=out_fit,
        geometry_v9d2=dict(
