...
#!/usr/bin/env python3
# arc_task_parser_geodesic_v3_fix.py
# Same as v3, but fixes the gradient sign for V = -1/r (grad V = +(x-c)/r^3).
# Also trims smoothing a bit to make spikes visible and sets a slightly lower energy threshold.
...
# Parsing thresholds
ABLATION_TAU = 0.02
ENERGY_TAU   = 0.15       # slightly lower now that energy is correct
ENERGY_MARGIN = 0.10
MIN_DUR_STEPS_FRAC = 0.01
SMOOTH_WIN   = 5          # smaller window to preserve narrow spikes
CONC_COS_TAU = 0.25
...
def run_geodesic(x0: np.ndarray, centers: np.ndarray, include_mask: np.ndarray):
    ...
    for t in range(STEPS):
        ...
        # With correct grad sign, -(grad · v) is positive when moving toward a center.
        raw = -(grads @ v)
        raw = np.clip(raw, 0.0, None)
        P[t] = math.exp(2.0 * lnphi) * raw
    ...
    for i, (z0, true) in enumerate(zip(Zte, yte), 1):
        ...
        if PLOT:
            try:
                ...
                fname = os.path.join(outdir, f"parse_energy_v3fix_{tag}_{ts}.png")
