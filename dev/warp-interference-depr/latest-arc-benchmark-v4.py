# Patch the parser to maintain constant metric speed and fix ablation sign,
# and weight aligned power by metric factor. It writes a new file v2.
# from textwrap import dedent, indent
# code = dedent(r"""
#!/usr/bin/env python3
# arc_task_parser_geodesic_v2.py
# Parse-only (v2): constant metric-speed geodesic + ablation sign fix + metric-weighted aligned energy.
...
# Geometry
LAMBDA = 0.35
GAMMA  = 0.00          # no damping for parse-only; maintain constant metric speed
DT     = 0.02
STEPS  = 700
EPS    = 1e-9
SPEED_METRIC2 = 1.0    # target φ(x)^2 ||v||^2

# Parsing thresholds
ABLATION_TAU = 0.02    # smaller, since ΔS now meaningful
ENERGY_TAU   = 0.18
ACT_THRESH   = 0.35
MIN_DUR_FRAC = 0.06
CONC_COS_TAU = 0.25
...
def rk4_step_const_speed(x: np.ndarray, v: np.ndarray, dt: float, centers: np.ndarray, include_mask: np.ndarray):
    def acc(x_, v_):
    ...
    # renormalize to constant metric speed
    lnphi_new, _ = lnphi_and_grad(x_new, centers, include_mask)
    vs2 = metric_speed2(x_new, v_new, lnphi_new)
    if vs2 <= 0.0 or not np.isfinite(vs2):
        # reset along steepest descent if degenerate
        _, gL = lnphi_and_grad(x_new, centers, include_mask)
        gdir = -gL; n = np.linalg.norm(gdir) + 1e-9
        v_new = (math.sqrt(SPEED_METRIC2) * np.exp(-lnphi_new)) * (gdir / n)
    else:
        scale = math.sqrt(SPEED_METRIC2 / vs2)
        v_new = v_new * scale
    return x_new, v_new
...
def run_geodesic(x0: np.ndarray, centers: np.ndarray, include_mask: np.ndarray):
    ...
    # pick v0 to satisfy metric speed target
    lnphi0, _ = lnphi_and_grad(x0, centers, include_mask)
    v = (math.sqrt(SPEED_METRIC2) * math.exp(-lnphi0)) * (dir0 / n0)

    x = x0.copy()
    S = 0.0
    P = np.zeros((STEPS, K), dtype=np.float64)  # aligned power (metric-weighted)
    ...
    for t in range(STEPS):
        ...
        raw = -(grads @ v)                     # alignment with motion
        raw = np.clip(raw, 0.0, None)
        P[t] = math.exp(2.0 * lnphi) * raw     # metric-weighted aligned power

        norms = np.linalg.norm(grads, axis=1) + 1e-9
        dirs  = grads / norms[:, None]
        RHO[t] = dirs @ dirs.T

        x, v = rk4_step_const_speed(x, v, DT, centers, include_mask)
...
def parse_from_timeseries(P: np.ndarray, RHO: np.ndarray, labels: List[str],
                          act_thresh: float = ACT_THRESH, min_dur_frac: float = MIN_DUR_FRAC,
                          conc_cos_tau: float = CONC_COS_TAU):
    T, K = P.shape
    row_sums = P.sum(axis=1, keepdims=True) + 1e-9
    Q = P / row_sums
    active = Q >= act_thresh
    min_len = max(1, int(min_dur_frac * T))
    for k in range(K):
        seg = active[:,k].astype(np.int32)
        i = 0
        while i < T:
            if seg[i] == 1:
                j = i
                while j < T and seg[j] == 1: j += 1
                if (j - i) < min_len:
                    active[i:j, k] = False
                i = j
            else:
    ...
    present_idxs = [k for k in range(K) if active[:,k].any()]
    present = [labels[k] for k in present_idxs]

    starts = []
    for k in present_idxs:
        idxs = np.where(active[:,k])[0]
        starts.append((k, int(idxs[0])))
    starts.sort(key=lambda z: z[1])
    order = [labels[k] for k,_ in starts]

    concurrency = []
    for i in range(len(present_idxs)):
        for j in range(i+1, len(present_idxs)):
            ki, kj = present_idxs[i], present_idxs[j]
            overlap = active[:,ki] & active[:,kj]
            if overlap.any():
                idxs = np.where(overlap)[0]
                mean_abs_rho = float(np.mean(np.abs(RHO[idxs, ki, kj])))
                if mean_abs_rho < conc_cos_tau:
                    concurrency.append((labels[ki], labels[kj]))

    return present, order, concurrency

def ablation_parse(x0: np.ndarray, centers: np.ndarray, labels: List[str]):
    K = centers.shape[0]
    include_all = np.ones(K, dtype=bool)
    S_all, P_all, RHO_all = run_geodesic(x0, centers, include_all)

    deltas = []
    for k in range(K):
        mask = include_all.copy(); mask[k] = False
        S_drop, _, _ = run_geodesic(x0, centers, mask)
        deltas.append(S_drop - S_all)  # ΔS_k = S_{-k} - S_all (positive => k required)
    deltas = np.array(deltas, dtype=np.float64)

    P_s = smooth_ma(P_all, SMOOTH_WIN)
    E = P_s.sum(axis=0)
    E_frac = E / (E.sum() + 1e-9)

    dfrac = deltas / (abs(S_all) + 1e-9)

    present = [labels[k] for k in range(K) if (dfrac[k] > ABLATION_TAU) or (E_frac[k] > ENERGY_TAU)]
    present_ts, order, conc = parse_from_timeseries(P_s, RHO_all, labels)
    present_union = list(dict.fromkeys(present + present_ts))
    ...
    return {
        ...
        "dfrac": dfrac.tolist(),
        "E": E.tolist(),
        "E_frac": E_frac.tolist(),
        "present": present_union,
        "order": order,
        "concurrency": conc,
        "P_s": P_s,
    }
...
def run(device: str = DEVICE_DEFAULT, outdir: str = "."):
    ...
    for i, (z0, true) in enumerate(zip(Zte, yte), 1):
        tag = f"sample{i:02d}"
        res = ablation_parse(z0, centers, LABELS)
        present = res["present"]; order = res["order"]; conc = res["concurrency"]
        conc_str = " | ".join([f"{a} ∥ {b}" for (a,b) in conc]) if conc else "none"
        print(f"[{i:02d}] true={true:<7} -> Tasks: {present} | Order: {' → '.join(order) if order else '—'} | Concurrency: {conc_str}")
        print(f"      ΔS: {np.array(res['deltas']).round(3).tolist()}  dfrac: {np.array(res['dfrac']).round(3).tolist()}  Efrac: {np.array(res['E_frac']).round(3).tolist()}")
        ...
        if PLOT:
            try:
                ...
                for k, name in enumerate(LABELS):
                    plt.plot(res["P_s"][:,k], label=name)
                plt.xlabel("step"); plt.ylabel("aligned power (metric, smoothed)")
                plt.title(f"Field-aligned energy — {tag}")
                plt.legend()
                ts = time.strftime("%Y%m%d-%H%M%S")
                fname = os.path.join(outdir, f"parse_energy_v2_{tag}_{ts}.png")
...
if __name__ == "__main__":
    main()
# """)
# path = "/mnt/data/arc_task_parser_geodesic_v2.py"
