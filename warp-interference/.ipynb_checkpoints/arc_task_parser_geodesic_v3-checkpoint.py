...
#!/usr/bin/env python3
# arc_task_parser_geodesic_v3.py
# Parse-only (v3): constant Euclidean speed (so action varies), improved presence rule,
# peak-based ordering, short-duration tolerance.
...
SPEED_EUCLID = 0.12   # keep ||v|| = const so S = ∫ φ^2 ||v||^2 dt varies with φ

# Parsing thresholds
ABLATION_TAU = 0.02       # fractional ΔS threshold
ENERGY_TAU   = 0.20       # base E_frac threshold
ENERGY_MARGIN = 0.12      # tasks within margin of top E_frac are also present
MIN_DUR_STEPS_FRAC = 0.01 # allow short spikes
SMOOTH_WIN   = 9
...
def pick_present_from_energy(E_frac: np.ndarray, dfrac: np.ndarray, labels: List[str]) -> List[str]:
    top = float(E_frac.max(initial=0.0))
    cand = [i for i,e in enumerate(E_frac) if (e >= max(ENERGY_TAU, top - ENERGY_MARGIN))]
    # include any with strong ablation too
    strong = [i for i,d in enumerate(dfrac) if d > ABLATION_TAU]
    idxs = sorted(set(cand).union(strong))
    return [labels[i] for i in idxs]

def peak_order(Ps: np.ndarray, present: List[str], labels: List[str]) -> List[str]:
    # order by time of peak aligned power (smoothed)
    order = []
    for name in present:
        k = labels.index(name)
        t_peak = int(np.argmax(Ps[:,k]))
        order.append((t_peak, name))
    order.sort()
    return [n for _,n in order]

def parse_concurrency(Ps: np.ndarray, present: List[str], labels: List[str], rho: np.ndarray):
    # half-maximum windows around peaks; overlap + low |rho| -> concurrency
    T = Ps.shape[0]
    peaks = {}
    for name in present:
        k = labels.index(name)
        pk = float(Ps[:,k].max(initial=0.0))
        t0 = int(np.argmax(Ps[:,k]))
        half = 0.5 * pk
        # expand left/right
        L = t0
        while L > 0 and Ps[L-1,k] >= half: L -= 1
        R = t0
        while R < T-1 and Ps[R+1,k] >= half: R += 1
        peaks[name] = (L,R)
    conc = []
    for i in range(len(present)):
        for j in range(i+1, len(present)):
            a,b = present[i], present[j]
            Li,Ri = peaks[a]; Lj,Rj = peaks[b]
            overlap = not (Ri < Lj or Rj < Li)
            if overlap:
                # average |rho| in overlap
                ki, kj = labels.index(a), labels.index(b)
                idxs = np.arange(max(Li,Lj), min(Ri,Rj)+1)
                mean_abs = float(np.mean(np.abs(rho[idxs, ki, kj]))) if len(idxs)>0 else 1.0
                if mean_abs < CONC_COS_TAU:
                    conc.append((a,b))
    return conc
...
def ablation_parse(x0: np.ndarray, centers: np.ndarray, labels: List[str]):
    ...
    for k in range(K):
        ...
        deltas.append(S_drop - S_all)  # positive => k required
    deltas = np.array(deltas, dtype=np.float64)

    Ps = smooth_ma(P_all, SMOOTH_WIN)
    E = Ps.sum(axis=0)
    E_frac = E / (E.sum() + 1e-9)

    dfrac = deltas / (abs(S_all) + 1e-9)

    present = pick_present_from_energy(E_frac, dfrac, labels)
    # allow short windows: lower minimum duration to a few steps
    order = peak_order(Ps, present, labels)
    conc = parse_concurrency(Ps, present, labels, RHO_all)
    ...
    for i, (z0, true) in enumerate(zip(Zte, yte), 1):
        ...
        if PLOT:
            try:
                ...
                fname = os.path.join(outdir, f"parse_energy_v3_{tag}_{ts}.png")
