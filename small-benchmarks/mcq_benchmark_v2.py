#!/usr/bin/env python3
# mcq_benchmark_v2.py — stock → geo → geo+detect → geo+detect+denoise @ tap -9
import os, csv, json, time, argparse, random, subprocess, sys, re
from pathlib import Path
from collections import Counter

"""
# STOCK (generates prompts/truths once)
python3 mcq_benchmark_v2.py --runner text_arc_unified_base.py \
  --device cuda --model gpt2-large --tap -9 --profile stock \
  --n 60 --k_shot 5 --max_new_tokens 1 --outdir mcq_run

# GEO
python3 mcq_benchmark_v2.py --runner text_arc_unified_base.py \
  --device cuda --model gpt2-large --tap -9 --profile geo \
  --prompts_in mcq_run/final_prompts.txt --truths_in mcq_run/final_truths.csv \
  --outdir mcq_run

# GEO + DETECT (gain-only)
python3 mcq_benchmark_v2.py --runner text_arc_unified_base.py \
  --device cuda --model gpt2-large --tap -9 --profile geo_detect \
  --prompts_in mcq_run/final_prompts.txt --truths_in mcq_run/final_truths.csv \
  --outdir mcq_run

# GEO + DETECT + DENOISE (soft)
python3 mcq_benchmark_v2.py --runner text_arc_unified_base.py \
  --device cuda --model gpt2-large --tap -9 --profile geo_detect_denoise \
  --prompts_in mcq_run/final_prompts.txt --truths_in mcq_run/final_truths.csv \
  --outdir mcq_run

"""

DEFAULT_RUNNER = "/mnt/data/text_arc_unified_base.py"  # same runner you used
FIELD_CANDIDATES = ["generation","prediction","output","text","decoded","answer","completion"]
LETTER_RE = re.compile(r"\b([A-D])\b", re.IGNORECASE)

# ---- v4b tap-9 baseline (T4/L4) ----
V4B_BASE = dict(
    alpha0="0.05", alpha_min="0.006",
    trend_tau="0.35", k_tr="12",
    detect_width="24", detect_sigma="5",
    null_K="32", null_q="0.92", k_det="7",
    s_latch="0.30", linger="2", ema_center_beta="0.05",
)
PROFILE_JSON = "/mnt/data/calib/profile_v4b_tap9_text.json"  # use if runner supports --config
...
def mcq_min_of_three(rng):
    ...
    while len(pool) < 3:
        x = rng.randint(0, 120)
        if x != correct and x not in pool: pool.append(x)
...
def write_prompts_truths(outdir: Path, items, stem="final", prefix=""):
    ...
    with open(prompts_path,"w",encoding="utf-8") as f:
        for _,p,_,_,_ in items: f.write((prefix+p).strip()+"\n")
...
def runner_cmd_base(runner, gen_mode, prompts_path, out_jsonl, tap, device, model, max_new_tokens):
...
def add_v4b_flags(cmd, use_detect=False, use_denoise=False):
    # Prefer a frozen config if present (recommended in the Steps 1–10 doc)
    if os.path.exists(PROFILE_JSON):
        cmd += ["--config", PROFILE_JSON]
        # Still allow light overrides if needed:
        cmd += ["--use_detect", "1" if use_detect else "0"]
        cmd += ["--use_denoise", "1" if use_denoise else "0"]
        return cmd

    # Fallback to explicit flags (v4b baseline @ tap -9), WITHOUT detect_sigma
    for k,v in V4B_BASE.items():
        # Skip detect_sigma entirely; runner doesn't support it
        if k == "detect_sigma":
            continue
        cmd += [f"--{k}", str(v)]
...
def run_profile(runner, profile, prompts_path, out_jsonl, tap, device, model, max_new_tokens, auto_cpu_fallback=True):
    if profile == "stock":
        cmd = runner_cmd_base(runner, "stock", prompts_path, out_jsonl, tap, device, model, max_new_tokens)
    elif profile == "geo":
        cmd = runner_cmd_base(runner, "geo",   prompts_path, out_jsonl, tap, device, model, max_new_tokens)
    ...
    if p.returncode != 0 and auto_cpu_fallback and ("CUDA error" in (p.stderr or "") or "device-side assert" in (p.stderr or "")):
        print("[WARN] CUDA error; retrying on CPU...", file=sys.stderr)
        if "--device" in cmd:
            i = cmd.index("--device"); cmd[i+1] = "cpu"
        else:
            cmd += ["--device","cpu"]
        print("[RUN]"," ".join(cmd))
        p = subprocess.run(cmd, capture_output=True, text=True)
...
def main():
    ap=argparse.ArgumentParser(description="Unified MCQ benchmark (stock → geo → geo+detect → geo+detect+denoise)")
    ap.add_argument("--runner", type=str, default=DEFAULT_RUNNER)
    ap.add_argument("--device", type=str, default=None)
    ap.add_argument("--model",  type=str, default="gpt2")
    ap.add_argument("--tap",    type=int, default=-9)
    ap.add_argument("--profile",type=str, default="stock",
                    choices=["stock","geo","geo_detect","geo_detect_denoise"])
    ap.add_argument("--n",      type=int, default=50)
    ap.add_argument("--seed",   type=int, default=42)
    ap.add_argument("--k_shot", type=int, default=5)
    ap.add_argument("--max_new_tokens", type=int, default=1)
    ap.add_argument("--outdir", type=str, default="/mnt/data/mcq_bench_llm")
    ...
    if args.prompts_in and args.truths_in:
        prompts_path=args.prompts_in; truths_csv=args.truths_in
    ...
    gens_jsonl=str(outdir / f"generations_{args.profile}.jsonl")
    run_profile(args.runner, args.profile, prompts_path, gens_jsonl, args.tap,
                args.device, args.model, args.max_new_tokens, auto_cpu_fallback=(not args.no_cpu_fallback))

    items=read_jsonl(gens_jsonl)
    per_item, metrics=score(items, truths_csv)

    items_csv=str(outdir / f"items_{args.profile}.csv")
    with open(items_csv,"w",encoding="utf-8",newline="") as f:
        w=csv.DictWriter(f, fieldnames=["id","prediction","truth","family","correct"])
        w.writeheader(); [w.writerow(r) for r in per_item]
    print(f"[CSV]  {items_csv}")

    metrics_json=str(outdir / f"metrics_{args.profile}.json")
    with open(metrics_json,"w",encoding="utf-8") as f: json.dump(metrics,f,indent=2)
    print(f"[JSON] {metrics_json}")

    metrics_csv=str(outdir / f"metrics_{args.profile}.csv")
    with open(metrics_csv,"w",encoding="utf-8",newline="") as f:
        ...
        w.writeheader(); w.writerow({k:metrics.get(k,"") for k in ["n_scored","tp","fp","fn","accuracy","precision","recall","f1","hallucination_rate","omission_rate","match_mode","pred_field_used"]})
    print(f"[CSV]  {metrics_csv}")

    out_json=str(outdir / f"generations_{args.profile}.json")
    with open(out_json,"w",encoding="utf-8") as f: json.dump(items,f,indent=2)
    print(f"[JSON] {out_json}")

    print(json.dumps({"SUMMARY": {args.profile.upper(): metrics}}, indent=2))
