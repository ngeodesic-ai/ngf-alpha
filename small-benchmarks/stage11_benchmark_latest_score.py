...
def score_latents_prompt_local(H: np.ndarray, well_cfg=None, gamma=0.5):
    """
    H: (M, D) candidate vectors for one prompt.
    Returns:
      scores: list of dict(depth, margin, gates_ok, conf)
      pick: index of chosen candidate (by depth + gamma*margin) among gate-passing, else best anyway
    """
    # 1) Warp the batch into single-well space (reuses Stage-11)
    X3, metrics, info = pca3_and_warp(H, energy=None, params=WellParams(**(well_cfg or {})))
    z = X3[:, 2]                    # more negative == deeper
    order = np.argsort(z)           # z[order[0]] is deepest
    ...
    # 2) Per-candidate fields
    depth = (-z).astype(float)                          # flip sign so higher is better
    depth_norm = (depth - depth.min()) / (depth.ptp() + 1e-9)

    # 3) Gates (prompt-level + candidate-level)
    #    - strong prompt well (phantom_index + margin)
    gate_prompt = (metrics.get("phantom_index", 0.0) >= 0.12) and (margin_prompt >= 0.06)
    #    - candidate must be in lower quartile of z (top quartile of depth)
    z_q25 = float(np.quantile(z, 0.25))
    gates_ok = [(gate_prompt and (z[i] <= z_q25)) for i in range(len(z))]

    # 4) Composite and pick
    gaps = np.array([float(z[i] - z[best]) for i in range(len(z))])  # best=0, others >0
    comp = depth_norm + gamma * (gaps if gaps.ndim else 0.0)
    if any(gates_ok):
        pick = int(np.argmax(np.where(gates_ok, comp, -1e9)))
    else:
        pick = int(np.argmax(comp))  # abstain upstream if you like

    # 5) Confidence (squash depth)
    conf = 1.0 / (1.0 + np.exp(-5*(depth_norm - depth_norm.mean())))

    scores = [dict(depth=float(depth_norm[i]),
                   margin=float(gaps[i]),
                   gates_ok=bool(gates_ok[i]),
                   conf=float(conf[i]))
              for i in range(len(z))]
    return scores, pick, dict(phantom_index=float(metrics.get("phantom_index", 0.0)),
