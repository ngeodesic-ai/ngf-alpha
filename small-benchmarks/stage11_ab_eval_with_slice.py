...
class TerraformHook(nn.Module):
    def __init__(self, layer_module: nn.Module, calib: Calib, alpha_max: float, device: torch.device, verbose: bool=False):
        ...
        self.trace: List[torch.Tensor] = []  # all last-token hiddens across steps
    ...
    def enable(self):
        ...
        def _hook(module, inputs, output):
            try:
                ...
                for b in range(B):
                    h_b = last[b].detach()
                    self.buf.append(h_b.to(self.device))
                    self.trace.append(h_b.cpu())
                # keep last-k
...
def run_ab(args):
    ...
    for i, p in enumerate(prompts, 1):
        ...
        # Dump per-prompt hidden trace and collect for global PCA slice
        try:
            if len(hook.trace) > 0:
                H = torch.stack(hook.trace, dim=0).cpu().numpy()
                all_hidden.append(H)
                if args.dump_hidden:
                    os.makedirs(args.dump_hidden, exist_ok=True)
                    np.save(os.path.join(args.dump_hidden, f"hidden_layer{args.layer}_p{i:02d}.npy"), H)
        except Exception as e:
            print("[WARN] dump_hidden failed:", repr(e))
    ...
    # Optional: render a global 2D PCA slice from collected hidden states
    if args.render_pca_slice and plt is not None and len(all_hidden) > 0:
        try:
            X = np.vstack(all_hidden)  # (N,D)
            ...
            bins = 120
            H2, xedges, yedges = np.histogram2d(Z[:,0], Z[:,1], bins=bins)
            Xg = 0.5*(xedges[:-1] + xedges[1:])
            Yg = 0.5*(yedges[:-1] + yedges[1:])
            XG, YG = np.meshgrid(Xg, Yg, indexing='xy')

            fig, ax = plt.subplots(figsize=(7,6))
            cs = ax.contourf(XG, YG, H2.T, levels=14, alpha=0.85)
            fig.colorbar(cs, ax=ax, shrink=0.8, label='density')

            # overlay per-prompt trajectories colored by time
            start = 0
            for Hprompt in all_hidden:
                Zp = (Hprompt - mu) @ PCs  # (Tp,2)
                t = np.linspace(0, 1, len(Zp))
                ax.plot(Zp[:,0], Zp[:,1], linewidth=1.6, alpha=0.9)
                # mark start/end
                ax.scatter([Zp[0,0]],[Zp[0,1]], s=24, marker='o', alpha=0.9)
                ax.scatter([Zp[-1,0]],[Zp[-1,1]], s=36, marker='*', alpha=0.9)
                start += len(Zp)

            ax.set_xlabel('PC1'); ax.set_ylabel('PC2')
            ax.set_title(f'Layer {args.layer} — PCA slice of last-token hidden traces')
            ax.axhline(0, color='k', linewidth=0.5, alpha=0.3)
            ax.axvline(0, color='k', linewidth=0.5, alpha=0.3)
            plt.tight_layout()
            fig.savefig(args.slice_out, dpi=200)
            plt.close(fig)
            print(f"[SLICE] 2D PCA slice saved to {args.slice_out}")
        except Exception as e:
            print("[WARN] render_pca_slice failed:", repr(e))
    elif args.render_pca_slice and plt is None:
        print("[WARN] Matplotlib not available; cannot render PCA slice.")
...
def build_argparser():
    ...
    ap.add_argument('--dump_hidden', type=str, default='', help='If set, dump per-prompt last-token hidden states (.npy) to this directory')
    ap.add_argument('--render_pca_slice', action='store_true', help='Render a 2D PCA slice + density from collected hidden traces')
    ap.add_argument('--slice_out', type=str, default='pca_slice.png', help='Output PNG for --render_pca_slice')
