...
for S in "${SEEDS[@]}"; do
  python3 micro_llm_v4.py \
    --data bench/runs/latent_arc_eval_seed${S}.npz \
    --num_classes 5 --batch_size 256 \
    --epochs 0 --eval_only \
    --load_ckpt ckpts/micro_latent_arc_jit.pth \
    --load_stats stats/micro_stats_jit.npz \
    bench/runs/micro_latent_arc_seed${S}.log
done
...
class LatentARCDataset(Dataset):
    def __init__(self, X, y, seed_jitter=0, jitter_sigma=0.0):
        self.X = X
        self.y = y
    ...
    def __len__(self):
        return len(self.X) * (1 + self.seed_jitter)

    def __getitem__(self, idx):
        base_idx = idx % len(self.X)
        x = self.X[base_idx].copy()
        y = self.y[base_idx]
        if idx >= len(self.X) and self.jitter_sigma > 0.0:
            noise = np.random.normal(0, self.jitter_sigma, size=x.shape).astype(np.float32)
            x = x + noise
...
if __name__ == "__main__":
    ...
    parser.add_argument("--seed_jitter", type=int, default=0,
                        help="number of jittered clones per sample (0 = off)")
    parser.add_argument("--jitter_sigma", type=float, default=0.0,
                        help="stddev of Gaussian jitter for augmentation")
    ...
    train_ds = LatentARCDataset(Xtr, Ytr,
