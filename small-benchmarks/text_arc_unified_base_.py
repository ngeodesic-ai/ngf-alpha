...
def generate_one(model, tokenizer, device, prompt: str, max_new_tokens: int, temperature: float, top_p: float, do_sample: bool) -> str:
    ...
    return tokenizer.decode(gen_ids[0][toks["input_ids"].shape[1]:], skip_special_tokens=True)
...
def main():
    ...
    with open(args.out, "w", encoding="utf-8") as fout:
        for i,p in enumerate(eval_prompts,1):
            ...
            text = generate_one(model, tok, device, p, args.max_new_tokens, args.temperature, args.top_p, args.do_sample)
            telemetry.flush()
