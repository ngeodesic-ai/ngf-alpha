...
# -*- coding: utf-8 -*-
"""
Stage-11 A/B Eval — PLUS (always-on warp, soft gate, gen_mode, convergence metrics)
-------------------------------------------------------------------------------
- Always-on curvature: α_min > 0 applied every token.
- Soft gain (no hard gate): s_t ∈ [0,1] from sigmoids on trend ± optional Detect.
- Optional Detect acts only as a *multiplier* (gain control), never permission.
- Micro-burst support via linger/latch.
- Optional EMA center in PCA-2 so "inward" stays meaningful per prompt.
- NEW: --gen_mode {stock|geo} to decode under warp if desired.
- NEW: Convergence metrics: mean radius shrink, start/end radius.
- NEW: Burst metrics: mean run length, adjacency ratio; dlp_in/out split by s ≥ burst_thr.
- NEW: Step-size clip (relative) via --eps to bound ∥Δh∥/∥h∥.
- Improved HOOK logging: prints g_tr and g_det along with s and alpha.


Example (BASE) Warp + soft Detect and T4 / L4 budget GPU 
    python3 stage11_ab_eval_base.py \
      --model gpt2 --layer -9 \
...
Example (thesis‑pure):
  python3 stage11_ab_eval_base.py \
    ...
    --trend_tau 0.30 --k_tr 10 --use_detect 0 \
    --linger 3 --s_latch 0.7 --ema_center_beta 0.05 \
    --gen_mode stock --out_json ab_results.json

Example (generate under warp + soft Detect):
  python3 stage11_ab_eval_base.py \
    --model gpt2 --layer -9 \
    --prompts wobble_prompts_v1.txt --max_new_tokens 96 \
    --alpha0 0.07 --alpha_min 0.012 \
    ...
    --linger 3 --s_latch 0.7 --ema_center_beta 0.05 \
    --gen_mode geo --out_json ab_results_geo.json

CPU-friendly (proof runs / quick sanity)
    python3 stage11_ab_eval_base.py \
      --model gpt2 --layer -9 \
      --prompts wobble_prompts_v1.txt --max_new_tokens 64 \
      --alpha0 0.06 --alpha_min 0.00 \
      --trend_tau 0.60 --k_tr 7 \
      --use_detect 0 \
      --linger 1 --s_latch 0.50 --ema_center_beta 0.05 \
      --eps 0.25 --burst_thr 0.60 \
      --gen_mode stock --print_every 128 \
      --out_json ab_cpu.json
