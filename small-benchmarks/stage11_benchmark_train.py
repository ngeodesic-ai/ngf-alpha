...
python3 stage11_benchmark_train.py \
          --samples 20000 --seed 42 \
          --denoise_mode hybrid --ema_decay 0.85 --median_k 3 \
          --probe_k 5 --probe_eps 0.02 --conf_gate 0.65 --noise_floor 0.03 \
          --latent_arc --latent_dim 64 --latent_arc_noise 0.05 \
          --out_json latent_arc_denoise_20k.json \
          --dump_latents latent_arc_20k.npz
...
class Runner:
    def __init__(self, args: argparse.Namespace, hooks: ModelHooks):
        ...
        self._dump = [] if getattr(args, "dump_latents", "") else None
    ...
    def run_sample(self, idx: int) -> Dict[str, float]:
        ...
        if self._dump is not None:
            self._dump.append( (x_t.copy(), x_star.copy(), self._last_latent_arc_name) )
    ...
    def run(self) -> Dict[str, float]:
        ...
        if self._dump is not None:
            ...
            np.savez(self.args.dump_latents,
                     x0=np.stack(x0), x_star=np.stack(xs),
                     label=np.array([labmap[k] for k in nm], int),
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--dump_latents", type=str, default="", help="NPZ path to dump (x0,x_star,label,name)")
