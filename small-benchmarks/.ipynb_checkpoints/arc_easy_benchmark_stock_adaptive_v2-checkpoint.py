#!/usr/bin/env python3
"""
arc_easy_benchmark_stock_adaptive_v2.py — STOCK-only adaptive/easy benchmark (v2)
----------------------------------------------------------------------------------
Upgrades:
  • --fixed_mix '{"echo_integer":0.5,"extract_last":0.25,"pick_larger":0.25}'
    to bypass calibration and force a mix
  • Wider calibration sweep (8 mixes from very easy → moderate)
  • --k_shot to control number of inlined few-shots (1..10)


python3 arc_easy_benchmark_stock_adaptive_v2.py \
  --n 120 --calib_n 24 --tap -9 --device cuda \
  --runner text_arc_unified_base.py \
  --k_shot 7 --outdir benchmark_results/llm_benchmark_stock_easy_adaptive_v2

python3 arc_easy_benchmark_stock_adaptive_v2.py \
  --runner text_arc_unified_base.py \
  --device cpu --tap -9 --model gpt2-medium \
  --n 120 --calib_n 24 --k_shot 7 \
  --outdir benchmark_results/llm_benchmark_stock_easy_adaptive_v2

python3 arc_easy_benchmark_stock_adaptive_v2.py \
  --runner text_arc_unified_base.py \
  --device cpu --tap -9 --model gpt2-small \
  --n 12 --calib_n 6 --k_shot 5 \
  --fixed_mix '{"echo_integer":0.60,"extract_last":0.20,"pick_larger":0.20}' \
  --outdir benchmark_results/stock_easy_fixed_smoke
"""
...
def make_fewshot(k_shot: int):
    # Repeat BASE_FEWSHOT cyclically to reach k_shot
    few = []
    i = 0
...
def calibrate_mix(args, base_mix, rng, outdir: Path, fewshot_pairs):
    """
    Sweep from very easy → moderate; pick the first hitting [target_min, target_max].
    """
    mixes = [
        {"echo_integer":0.70, "extract_last":0.20, "pick_larger":0.10},  # very easy
...
# ---------------- Main ----------------
def main():
    ap = argparse.ArgumentParser(description="STOCK-only adaptive easy benchmark (v2, targets ~30–40% accuracy)")
    ...
    ap.add_argument("--fixed_mix", type=str, default=None, help='JSON dict of weights to bypass calibration')
    ap.add_argument("--outdir", type=str, default="/mnt/data/arc_bench_stock_easy_adaptive_v2")
    ...
    if args.fixed_mix:
        try:
            chosen_mix = json.loads(args.fixed_mix)
        except Exception as e:
            raise SystemExit(f"--fixed_mix must be JSON, e.g. '{{\"echo_integer\":0.5,\"extract_last\":0.25,\"pick_larger\":0.25}}' (error: {e})")
        calib_metrics = {"note": "fixed_mix used; no calibration"}
    else:
        chosen_mix, calib_metrics = calibrate_mix(args, base_mix, rng, outdir, fewshot_pairs)

    # 2) Generate final tasks
    ...
    print(json.dumps({"SUMMARY": {"StockEasyAdaptiveV2": metrics, "ChosenMix": chosen_mix, "Calib": calib_metrics, "k_shot": args.k_shot}}, indent=2))
