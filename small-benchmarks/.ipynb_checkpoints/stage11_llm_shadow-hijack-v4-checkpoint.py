...
python3 stage11_llm_shadow-hijack-v4.py \
  ...
  --steps 128 \
  --eta 0.20 \
  --eta_cool 0.10 --eta_cool_start 0.60 \            # <- start cooling at 60% steps
  --ema_gamma 0.965 --med_k 19 \                      # <- stronger smoothing
  --jitter_sigma 0.018 --jitter_J 8 \
  --jitter_cool 0.0 --jitter_cool_start 0.80 \        # <- anneal jitter to zero in last 20%
  --backoff 0.85 \
  --tok_eta 0.20 \
  --use_depth_weighted_pi 1 --pi_beta 6.5 \
  --nms_radius 7 --sigma_scale 0.72 \                 # <- de-crowd + slightly stronger field
  --local_radius 1.35 \
