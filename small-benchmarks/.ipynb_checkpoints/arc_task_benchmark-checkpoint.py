# Create a harness that mirrors the CLI/output style of stage11-benchmark-latest.py
# for ARC task generation using our model runner.
#
# Features:
# - Modes: stock | geo (default: stock)
# - Uses text_arc_unified.py under the hood to generate JSONL
# - Writes --out_json and/or --out_csv in a "stage11-benchmark-like" schema
# - Optional truths CSV to compute accuracy; otherwise writes text-quality proxies
# - Records perf (elapsed, tokens, tokens/sec) if metrics JSON from runner is provided
#
# Usage examples:
#   # Stock @ tap -9 (no warp), write JSON+CSV in the benchmark style
#   python3 /mnt/data/arc_task_benchmark.py \
#     --mode stock \
#     --tap -9 \
#     --prompts /mnt/data/calib/ngf_eval_prompts_60.txt \
#     --out_json /mnt/data/arc_stock_tap9.json \
#     --out_csv  /mnt/data/arc_stock_tap9.csv
#
#   # Geo with profile (same prompts), plus truths scoring if available
#   python3 /mnt/data/arc_task_benchmark.py \
#     --mode geo \
#     --tap -9 \
#     --profile /mnt/data/calib/profile_v4b_tap9_text.json \
#     --prompts /mnt/data/calib/ngf_eval_prompts_60.txt \
#     --truths /mnt/data/truths.csv \
#     --out_json /mnt/data/arc_geo_v4b_tap9.json \
#     --out_csv  /mnt/data/arc_geo_v4b_tap9.csv
...
def main():
    ...
    ap.add_argument("--tap", type=int, default=-9, help="Tap layer metadata (no-op for stock)")
    ...
    if args.gen_jsonl is None:
        cmd = [sys.executable, RUNNER, "--prompts", args.prompts, "--out", tmp_gen]
        if args.mode == "geo":
            if args.profile:
