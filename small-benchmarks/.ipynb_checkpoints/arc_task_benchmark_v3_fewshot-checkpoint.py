#!/usr/bin/env python3
"""
arc_task_benchmark_v3_fewshot.py — STOCK-only with few-shot + QA prompt style
-----------------------------------------------------------------------------
What’s new vs v3:
- Adds a reusable FEWSHOT prefix (QA style) before each prompt
- Formats prompts as:
      Q: Compute 7+5.
      A:
  which GPT-2-style decoders often follow better
- Forces conservative decoding flags (if runner supports them):
      --temperature 0.0 --top_p 1.0 --max_new_tokens 8
- Keeps the same embedded scorer (accuracy/precision/recall/F1/hallucination/omission)

Usage (example):
  python3 /mnt/data/arc_task_benchmark_v3_fewshot.py --n 60 --tap -9 --device cuda \
      --runner /mnt/data/text_arc_unified_base.py \
      --outdir ./layer9_wiring_plan/llm_benchmark_stock_fewshot

python3 arc_task_benchmark_v3_fewshot.py \
  --n 60 --tap -9 --device cuda \
  --runner text_arc_unified_base.py \
  --outdir benchmark_results/llm_benchmark_stock_fewshot
"""
...
def generate_arc_tasks(n: int, seed: int = 42):
    ...
    for i in range(1, n+1):
        ...
        if kind == "add":
            ...
            core = f"Compute {a}*{b}."
            truth = a * b
        # QA-style wrapper improves adherence on small models
        prompt = "Q: " + core + "\nA:"
...
# --- normalization, reading, and scoring copied from v3 ---
...
def score(items, truths_csv, match_mode="numbers_only"):
    ...
    for obj in items:
        ...
        if n_words > 1:
            ...
            loopish = int(any(c>=6 for c in C2(toks).values()))
...
def main():
    ap = argparse.ArgumentParser(description="STOCK-only with few-shot QA-style prompts + embedded scoring")
    ...
    ap.add_argument("--outdir", type=str, default="/mnt/data/arc_bench_stock_fewshot")
