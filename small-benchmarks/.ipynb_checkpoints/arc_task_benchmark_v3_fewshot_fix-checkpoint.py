#!/usr/bin/env python3
"""
arc_task_benchmark_v3_fewshot_fix.py — STOCK-only, few-shot inlined + safe scoring
-----------------------------------------------------------------------------------
Fixes over v3_fewshot:
  • FEWSHOT exemplars are *inlined per prompt* (no stray prompts without truths)
  • Metrics use n_scored = #items with a ground-truth row
  • Still stock-only: no geo/detect/denoise

Usage:
  python3 arc_task_benchmark_v3_fewshot_fix.py \
    --n 60 --tap -9 --device cuda \
    --runner text_arc_unified_base.py \
    --outdir benchmark_results/llm_benchmark_stock_fewshot_fix
...
def score(items, truths_csv, match_mode="numbers_only"):
    ...
    # Metrics based only on items with truths
    n = n_scored
...
def main():
    ap = argparse.ArgumentParser(description="STOCK-only with *inlined* few-shot prompts + safe scoring")
    ...
    ap.add_argument("--outdir", type=str, default="/mnt/data/arc_bench_stock_fewshot_fix")
