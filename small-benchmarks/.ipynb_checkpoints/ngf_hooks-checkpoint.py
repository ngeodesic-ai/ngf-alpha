# ngf_hooks.py — Stage-1: Geo (warp-only)
...
def attach_ngf_hooks(
    ...
    *,
    ...
    cfg = dict(tap=tap, alpha0=alpha0, alpha_min=alpha_min,
    ...
    # Minimal per-run state
    state = {
        "ema_center": None,  # (C,)
        "burst": 0,          # latch/linger counter (prefers 2–3 token runs)
    }

    def ngf_forward_hook(module, inputs, output):
    # output can be Tensor or Tuple[Tensor, ...] from HF GPT-2 blocks
    is_tuple = isinstance(output, (tuple, list))
    x = output[0] if is_tuple else output            # (B, T, C)
    B, T, C = x.shape

    # --- 1) EMA center at tap (global across batch) ---
    seq_mean = x.mean(dim=1).mean(dim=0)  # (C,)
    if state["ema_center"] is None:
        state["ema_center"] = seq_mean.detach()
    else:
        state["ema_center"] = ema_center_beta * seq_mean.detach() + (1 - ema_center_beta) * state["ema_center"]
    ...
    # Re-wrap if the block returned a tuple
    if is_tuple:
