#!/usr/bin/env python3
# mcq_benchmark_stock_oneline_v2.py
...
# ------- MCQ generators (single-line prompts) -------
def mcq_pick_larger(rng):
    a = rng.randint(2, 99); b = rng.randint(2, 99)
    while b == a: b = rng.randint(2, 99)
    correct = max(a, b)

    # Build a pool of plausible distractors, explicitly EXCLUDING the correct answer
    ...
    letters = ["A","B","C","D"]
    assert correct in choices, "Internal error: correct value missing from choices"
    truth_letter = letters[choices.index(correct)]
    prompt = f"Q: Which is larger? A) {choices[0]} B) {choices[1]} C) {choices[2]} D) {choices[3]} Respond with exactly: The answer is <LETTER>."
...
def mcq_min_of_three(rng):
    ...
    correct = min(a,b,c)
    pool = list({a,b,c, correct+1, max(0, correct-1), a+b-c})
    ...
    truth_letter = letters[choices.index(correct)]
    prompt = f"Q: Return the smallest number. A) {choices[0]} B) {choices[1]} C) {choices[2]} D) {choices[3]} Respond with exactly: The answer is <LETTER>."
...
def mcq_add_single(rng):
    ...
    assert correct in choices
    truth_letter = letters[choices.index(correct)]
    prompt = f"Q: Compute {a}+{b}. A) {choices[0]} B) {choices[1]} C) {choices[2]} D) {choices[3]} Respond with exactly: The answer is <LETTER>."
...
def mcq_parity(rng):
    ...
    prompt = f"Q: Is {x} even or odd? A) {opts[0]} B) {opts[1]} C) {opts[2]} D) {opts[3]} Respond with exactly: The answer is <LETTER>."
...
def write_prompts_truths(outdir: Path, items, stem="final"):
...
def run_stock(runner, prompts_path, out_jsonl, tap, device=None, model=None, max_new_tokens="6", extra_args=None):
...
def main():
    ap = argparse.ArgumentParser(description="One-line MCQ STOCK benchmark (letter scoring, v2 fixed)")
    ...
    ap.add_argument("--outdir", type=str, default="mcq_bench_stock_oneline_v2")
    ...
    prompts_path, truths_csv, questions_csv = write_prompts_truths(outdir, tasks, stem="final")
    ...
    run_stock(args.runner, prompts_path, gens_jsonl, args.tap, device=args.device, model=args.model,
              max_new_tokens=str(args.max_new_tokens), extra_args=extra)
    ...
    print(json.dumps({"SUMMARY": {"StockMCQOneLineV2": metrics}}, indent=2))
