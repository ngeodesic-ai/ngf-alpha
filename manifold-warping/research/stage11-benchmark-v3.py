...
def parse_weights_map(s: str, default=(1.0,0.4,0.3)) -> Dict[str, Tuple[float,float,float]]:
...
def estimate_peak_indices_multi(S_channel: Dict[str, np.ndarray], protos: List[np.ndarray], width: int) -> Dict[str, int]:
    peak = {}
    for p, sig in S_channel.items():
        # use the first prototype for peak estimation (fast); energy stage uses all
        proto = protos[0]
        m = np.correlate(zscore(sig), proto, mode="same")
        peak[p] = int(np.argmax(m))
    return peak

def well_parser_step3(traces: Dict[str, np.ndarray],
                rng,
                sigma: int = 9,
                proto_width: int = 160,
                rank: int = 2,
    ...
    # residualize via common-mode then span complement
    ...
    peak_idx = estimate_peak_indices_multi(Sres_perp, PROTOS, proto_width)

    if weights_map is None:
        weights_map = {p: weights for p in keys}
    ...
    last_energies = {p: 0.0 for p in keys}
    ...
    def compute_energy(Sres_local, Sraw_local, Scm_local, peaks, lateral_peaks=None, picks_count=0):
        ...
        for p in keys:
            ...
            for proto in PROTOS:
                ...
                # raw floor penalty (encourage evidence in raw)
                if raw_floor is not None and z_raw < raw_floor:
                    Up += raw_floor_penalty * (raw_floor - z_raw)
                if best is None or Up < best:
                    best = Up
                    best_parts = (z_perp, z_raw, z_cm)
            Up = best
            # pick penalty
            if pick_penalty > 0.0:
                Up += pick_penalty * picks_count
            # lateral inhibition
            if lateral_peaks:
    ...
    while True:
        ...
        # margin
        false_U = [energies[p] for p in keys if p not in keep[:-1]]
        if len(false_U) > 1:
        ...
        # subtract aligned proto using the first proto (approx)
        t0 = peaks_work[pick]
        proto0 = PROTOS[0]
        ...
        peaks_work = estimate_peak_indices_multi(Sres_work, PROTOS, proto_width)
...
def main():
    ap = argparse.ArgumentParser(description="Stage 11 — Step 3: Prototype Ensemble & Per-Primitive Calibration")
    ...
    ap.add_argument("--weights_map", type=str, default="")
    ...
    ap.add_argument("--proto_agg", type=str, default="min")
    ap.add_argument("--raw_floor", type=float, default=None)
    ap.add_argument("--raw_floor_penalty", type=float, default=0.0)
    ap.add_argument("--pick_penalty", type=float, default=0.0)
    ap.add_argument("--csv", type=str, default="stage11_step3.csv")
    args = ap.parse_args()
    ...
    weights_map = parse_weights_map(args.weights_map, default=(w_perp, w_raw, w_cm))
    ...
    for i in range(1, args.samples+1):
        ...
        keep, order, energies, peaks, margins = well_parser_step3(
            traces, rng, sigma=args.sigma, proto_width=args.proto_width,
            rank=args.rank, weights=(w_perp, w_raw, w_cm), weights_map=weights_map, nperm=args.nperm,
            ...
            proto_modes=modes, proto_phases=phases, proto_agg=args.proto_agg,
            raw_floor=args.raw_floor, raw_floor_penalty=args.raw_floor_penalty,
            pick_penalty=args.pick_penalty
        )
    ...
    print("[Step3] Summary:", {k: (round(v,3) if isinstance(v,float) else v) for k,v in summary.items()})
    if margins_all:
        print(f"[Step3] Margins — mean={np.mean(margins_all):.3f}, min={np.min(margins_all):.3f}, n={len(margins_all)}")
