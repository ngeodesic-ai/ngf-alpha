...
# -*- coding: utf-8 -*-
"""
Stage 11 — Well Benchmark v9c2
------------------------------
Visual: Analytic 360° funnel with **finite-radius core** (no infinite spike) and optional **auto depth** to match point-cloud scale.
Parser: same as v9c (curvature coupling knobs remain; defaults keep κ=0.0 to avoid precision loss).

New CLI:
  --funnel_r0_frac        tip core radius as a fraction of r_max (default 0.14)
  --funnel_auto_depth     if set, compute D so z(0) lands a fixed margin below the data floor
  --funnel_depth_margin   margin as a fraction of (z_max - z_min) when auto depth is on

python3 stage11-well-benchmark-v9c2.py \
  ...
  --funnel_auto_depth --funnel_r0_frac 0.16 --funnel_p 1.6
...
def make_synthetic_traces(rng, T=720, noise=0.02, cm_amp=0.02, overlap=0.5,
    ...
    cm = cm_amp * (1.0 + 0.2 * np.sin(2*np.pi * np.arange(T) / max(30, T//6)))
...
# ---------- Analytic funnel with finite core ----------

def analytic_funnel(center, r_max, *, D=2.0, p=1.6, z_cap=0.0,
                    r0_frac=0.14, eps=1e-6, n_theta=160, n_r=220):
    """
    z(r) = z_cap - D * [ 1/(sqrt(r^2 + r0^2))^p - 1/(sqrt(r_max^2 + r0^2))^p ],  r0 = r0_frac * r_max
    Finite, smooth tip; no divergence as r -> 0.
    """
    r0 = r0_frac * max(r_max, eps)
    r_grid = np.linspace(0.0, r_max, n_r)
    invp = 1.0 / (np.sqrt(r_grid**2 + r0**2) + eps)**p
    invp_R = 1.0 / (np.sqrt(r_max**2 + r0**2) + eps)**p
    z_prof = z_cap - D * (invp - invp_R)
...
# ---------- Parser coupling (same as v9c; κ default 0) ----------
...
def geodesic_parse_curved(traces, *, sigma=9, proto_width=160, kappa=0.0, p=1.6, beta=0.4, q=2):
    keys = list(traces.keys())
    Eres = perpendicular_energy(traces)
    Sres = {pname: moving_average(Eres[pname], k=sigma) for pname in keys}

    r = radius_from_sample_energy(Sres)
    eps = 1e-6
    phi = 1.0 / np.power(r + eps, p)
    w   = 1.0 + beta * np.power(1.0 - r, q)

    Snew = {pname: w * Sres[pname] + kappa * phi for pname in keys}
...
def main():
    ap = argparse.ArgumentParser(description="Stage 11 — v9c2 finite-core funnel + curvature-coupled parsing")
    # data
    ...
    ap.add_argument("--funnel_D", type=float, default=1.2)
    ap.add_argument("--funnel_p", type=float, default=1.6)
    ap.add_argument("--funnel_cap", type=float, default=0.0)
    ap.add_argument("--funnel_r0_frac", type=float, default=0.14, help="core radius as a fraction of r_max")
    ap.add_argument("--funnel_auto_depth", action="store_true", help="auto-scale funnel depth to data range")
    ap.add_argument("--funnel_depth_margin", type=float, default=0.35, help="fraction of z-range to go below min when auto depth")
    ...
    # Funnel scale (auto depth optional)
    r_cloud = np.linalg.norm((X3_raw[:,:2] - info["center"]), axis=1)
    r_max = float(np.quantile(r_cloud, 0.98))
    D = args.funnel_D
    if args.funnel_auto_depth:
        z_min, z_max = float(z.min()), float(z.max())
        z_target = z_min - args.funnel_depth_margin * (z_max - z_min)
        # compute delta at r=0 for chosen r0
        r0 = args.funnel_r0_frac * max(r_max, 1e-6)
        invp0   = 1.0 / (np.sqrt(0.0**2 + r0**2) + 1e-6)**args.funnel_p
        invpR   = 1.0 / (np.sqrt(r_max**2 + r0**2) + 1e-6)**args.funnel_p
        delta = invp0 - invpR
        D = (args.funnel_cap - z_target) / max(delta, 1e-9)

    Xs, Ys, Zs, r_grid, z_prof = analytic_funnel(
        info["center"], r_max, D=D, p=args.funnel_p, z_cap=args.funnel_cap,
        r0_frac=args.funnel_r0_frac, n_theta=args.n_theta, n_r=args.n_r
    )
    out_funnel = args.out_plot.replace(".png", "_v9c2_funnel.png")
    plot_polar_surface(Xs, Ys, Zs, X3_raw, E, "Stage 11 — Analytic Funnel (v9c2, finite core)", out_funnel)
    ...
    summary = dict(
        ...
        plot_funnel=out_funnel,
        geometry_v9c2=dict(
            params=dict(kappa=args.kappa, p=args.p, beta=args.beta, q=args.q,
                        funnel_p=args.funnel_p, funnel_D=D, funnel_r0_frac=args.funnel_r0_frac,
                        funnel_auto_depth=bool(args.funnel_auto_depth), funnel_depth_margin=args.funnel_depth_margin)
        )
    )
    ...
    print(f"[PLOT] FUNNEL: {out_funnel}")
