...
# -*- coding: utf-8 -*-
"""
Stage 11 — Well Benchmark v9c3
------------------------------
Goal: Make the visual funnel pass *through* the point cloud by fitting a
radial z(r) profile from the data, while keeping a finite-core throat so the
shape still looks like the logo.

Parser path: same safe defaults as v9c2 (kappa=0.0 by default).
Viz path: data-fit radial profile (weighted quantile over r) + finite core + monotone.

Key knobs (viz):
  --fit_quantile     : percentile of z to fit at each radius (0.5 = median)
  --rbf_bw           : kernel bandwidth as fraction of r_max for weights
  --core_k, --core_p : finite-core strength and power
  --core_r0_frac     : core radius as fraction of r_max
  --blend_core       : 0..1 blend of (data-fit) with (finite-core template)

python3 stage11-well-benchmark-v9c3.py \
...
# ---------- data-fit radial profile (finite core) ----------
...
def weighted_quantile(values: np.ndarray, weights: np.ndarray, q: float) -> float:
    ...
    t = q * cumulative[-1]
    j = int(np.searchsorted(cumulative, t, side="left"))
...
def fit_radial_profile(X3: np.ndarray, center: np.ndarray, r_grid: np.ndarray,
    ...
    """Fit z(r) as weighted quantile of points at each radius; add finite-core."""
    ...
    for i, rg in enumerate(r_grid):
        ...
        if w.sum() < 1e-8:
            # fallback: nearest few points
            idx = np.argsort(np.abs(r - rg))[:8]
            z_fit[i] = float(np.median(z[idx]))
        else:
            z_fit[i] = weighted_quantile(z, w, q)

    # enforce monotone decrease toward center (more negative as r -> 0)
    # pass from rim -> center taking cumulative minimum
    last = z_fit[-1]
...
def analytic_core_template(r_grid: np.ndarray, D: float, p: float, r0_frac: float) -> np.ndarray:
    """Template finite-core profile normalized to zero at rim."""
    r_max = float(r_grid[-1] + 1e-12)
    r0 = r0_frac * r_max
...
def plot_fitted_surface(X, Y, Z, points: np.ndarray, energy: Optional[np.ndarray], title: str, out_path: str, alpha=0.9):
...
# ---------- parser (same as v9c2 safe defaults) ----------
...
def main():
    ap = argparse.ArgumentParser(description="Stage 11 — v9c3: data-fit radial funnel (finite core) + curvature-coupled parser")
    # data
    ...
    # viz (fit funnel)
    ap.add_argument("--fit_quantile", type=float, default=0.50, help="radial fit quantile in [0,1]")
    ap.add_argument("--rbf_bw", type=float, default=0.25, help="bandwidth as fraction of r_max")
    ...
    ap.add_argument("--blend_core", type=float, default=0.25, help="blend 0..1 toward template core")
    ap.add_argument("--template_D", type=float, default=1.2, help="template depth for blending")
    ap.add_argument("--template_p", type=float, default=1.6, help="template power for blending")
    ap.add_argument("--n_theta", type=int, default=160)
    ap.add_argument("--n_r", type=int, default=220)
    # parser coupling (safe defaults)
    ...
    out_fit = args.out_plot.replace(".png", "_v9c3_fit.png")
    plot_fitted_surface(Xs, Ys, Zs, X3_raw, E, "Stage 11 — Data-fit Funnel (v9c3)", out_fit)
    ...
    summary = dict(
        ...
        plot_fitted=out_fit,
        geometry_v9c3=dict(
            params=dict(fit_quantile=args.fit_quantile, rbf_bw=args.rbf_bw,
