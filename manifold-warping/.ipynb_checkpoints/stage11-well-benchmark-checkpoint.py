# Generate Stage 11 — Step 5 script with multi-scale/shape prototypes, per-primitive gating,
# prototype-family orthogonalization (for flip_v), and robust aggregation + consensus.


"""
python3 stage11-benchmark-well.py \
  --samples 100 \
  --seed 13 \
  --sigma 11 --proto_width 180 --rank 3 \
  --nperm 400 --block 64 --temp0 1.1 --temp_min 0.12 --anneal 0.55 \
  --inhib_strength 1.6 --inhib_sigma 72 \
  --presence_floor 0.55 \
  ...
  --raw_floor_map "flip_v:0.65" \
  --residual_ceiling_map "flip_v:1.00" \
  --consensus_k_map "flip_v:3" \
  --consensus_eps_map "flip_v:0.10" \
  --ortho_flip_v \
  --cm_amp 0.02 --overlap 0.50 --noise 0.02 --T 720 \
  --dump_surfaces_dir out_npz/ \
  --csv stage11_well.csv

python3 stage11-well-benchmark-v1.py \
  --profile S5-3 \
  --samples 50 \
  --dump_surfaces_dir dumps/surfaces_baseline \
  --dump_manifold dumps/manifold_baseline.npz \
  --pi
"""
...
def half_sine_scaled(width: int, scale: float = 1.0) -> np.ndarray:
    # scale < 1.0 -> narrower, > 1.0 -> wider; implemented by warping x
    x = np.linspace(0, 1, width)
    gamma = 1.0/ max(1e-6, scale)
    return np.sin(np.pi * (x ** gamma))
...
def skew_half_sine(width: int, skew: float = 0.5) -> np.ndarray:
    # skew in (0,1); <0.5 = left-heavy, >0.5 = right-heavy
    x = np.linspace(0, 1, width)
    beta = 3*(1-skew) + 1e-6
    return np.sin(np.pi * x**beta)

def hinge_left(width: int) -> np.ndarray:
    # sharp rise then abrupt fall (edge-like on left)
    x = np.linspace(0, 1, width)
    ramp = np.clip((x - 0.2) / 0.2, 0, 1)  # ramps up by 0.2→0.4
    plateau = np.clip(0.8 - x, 0, 1)       # quick fall near the end
    return np.minimum(ramp, plateau)

def hinge_right(width: int) -> np.ndarray:
    # mirror of hinge_left
    return hinge_left(width)[::-1]

def deriv_half_sine(width: int) -> np.ndarray:
    # derivative of sin over [0, pi] ~ cos; subtract mean to stabilize correlation
    t = np.linspace(0, np.pi, width)
    y = np.cos(t)
    return y

def normalize(p: np.ndarray, zero_mean: bool = True) -> np.ndarray:
    v = p.copy()
    if zero_mean:
        v = v - v.mean()
...
def gram_schmidt_orthonormalize(vecs: List[np.ndarray]) -> List[np.ndarray]:
    basis = []
    for v in vecs:
        u = v.copy()
        for b in basis:
...
def proj_residual(v: np.ndarray, basis: List[np.ndarray]) -> np.ndarray:
    u = v.copy()
    for b in basis:
        u = u - np.dot(u, b) * b
...
def generate_prototypes(width: int, modes: List[str], phases: List[float]) -> List[np.ndarray]:
    """Generate a list of normalized prototypes of equal length."""
    base = []
    for m in modes:
        if m == "half": base.append(half_sine(width))
        elif m == "halfN": base.append(half_sine_scaled(width, scale=0.5))  # narrow
        elif m == "halfW": base.append(half_sine_scaled(width, scale=2.0))  # wide
        ...
        elif m == "hingeL": base.append(hinge_left(width))
        elif m == "hingeR": base.append(hinge_right(width))
        elif m == "deriv": base.append(deriv_half_sine(width))
        else: base.append(half_sine(width))
    protos = []
    for b in base:
        b = normalize(b, zero_mean=True)
        for ph in phases:
            protos.append(normalize(shift_proto(b, ph), zero_mean=True))
    return protos
...
def set_metrics(true_list: List[str], pred_list: List[str]) -> Dict[str,float]:
    ...
    tp, fp, fn = len(Tset & Pset), len(Pset - Tset), len(Tset - Tset)  # fn computed next line
    fn = len(Tset - Pset)
...
def parse_map_list(s: str, default: List[str]) -> Dict[str, List[str]]:
    """Parse 'flip_v:half,hingeL;flip_h:half,triangle' into a dict of lists."""
    m = {p: list(default) for p in PRIMS}
    if not s: return m
    for part in s.split(";"):
        if not part.strip(): continue
        name, vals = part.split(":")
        m[name.strip()] = [v.strip() for v in vals.split(",") if v.strip()]
    return m
...
def well_parser_step5(traces: Dict[str, np.ndarray],
                rng,
                sigma: int = 9,
                proto_width: int = 160,
                rank: int = 2,
                ...
                proto_modes_map: Dict[str, List[str]] = None,
                ...
                consensus_k_map: Dict[str, int] = None,
                consensus_eps_map: Dict[str, float] = None,
                ortho_flip_v: bool = False,
    ...
    # prototypes per primitive
    phases = proto_phases or [0.0, 0.2]
    default_modes = ["half","triangle","skewL","skewR"]
    modes_map = proto_modes_map or {p: default_modes for p in keys}
    PROTOS = {p: generate_prototypes(proto_width, modes_map.get(p, default_modes), phases) for p in keys}

    # optional orthogonalization for flip_v against (flip_h ∪ rotate)
    if ortho_flip_v and "flip_v" in PROTOS:
        others = []
        for q in keys:
            if q != "flip_v":
                others.extend(PROTOS[q])
        basis = gram_schmidt_orthonormalize([normalize(v) for v in others])
        PROTOS["flip_v"] = [proj_residual(normalize(v), basis) for v in PROTOS["flip_v"]]

    # peak idx (rough) using flip_h's first proto (or any available)
    any_proto = PROTOS[keys[0]][0]
    peak_idx = estimate_peak_indices(Sres_perp, any_proto)

    if raw_floor_map is None: raw_floor_map = {p: None for p in keys}
    if residual_ceiling_map is None: residual_ceiling_map = {p: None for p in keys}
    if consensus_k_map is None: consensus_k_map = {p: 1 for p in keys}
    if consensus_eps_map is None: consensus_eps_map = {p: 0.15 for p in keys}
    if weights_map is None: weights_map = {p: (1.0,0.4,0.3) for p in keys}
    ...
    def aggregate_proto(vals: List[float], mode: str, tau: float) -> float:
    ...
    def compute_energy(Sres_local, Sraw_local, Scm_local, peaks, lateral_peaks=None, picks_count=0):
        ...
        for p in keys:
            ...
            Up_agg = aggregate_proto(parts, proto_agg, proto_tau)

            # consensus requirement: #protos within eps of Up_agg
            eps = consensus_eps_map.get(p, 0.15)
            kreq = int(consensus_k_map.get(p, 1))
            support = int(np.sum(np.array(parts) <= (Up_agg + eps)))
            if kreq > 0 and support < kreq:
                Up_agg += (kreq - support) * 0.5
    ...
    while True:
        ...
        # subtract aligned proto using a base half-sine
        t0 = peaks_work[pick]
        base_proto = normalize(half_sine(proto_width), zero_mean=True)
        M_work = subtract_aligned_proto(M_work, pick_k, t0, base_proto, proto_width)
        ...
        # re-estimate peaks using flip_h's first proto (rough)
        peaks_work = estimate_peak_indices(Sres_work, base_proto)
...
def main():
    ap = argparse.ArgumentParser(description="Stage 11 — Step 5: Prototype Redesign & Per-Primitive Gating")
    ...
    ap.add_argument("--proto_modes_map", type=str,
        default="flip_h:half,triangle,skewL,skewR;flip_v:half,halfN,halfW,hingeL,hingeR,deriv;rotate:half,triangle,skewR")
    ...
    ap.add_argument("--raw_floor_map", type=str, default="flip_v:0.60")
    ap.add_argument("--residual_ceiling_map", type=str, default="flip_v:1.10")
    ap.add_argument("--consensus_k_map", type=str, default="flip_v:2")
    ap.add_argument("--consensus_eps_map", type=str, default="flip_v:0.12")
    ap.add_argument("--ortho_flip_v", action="store_true")
    ap.add_argument("--pick_penalty", type=float, default=0.15)

    ap.add_argument("--csv", type=str, default="stage11_step5.csv")
    args = ap.parse_args()
    ...
    proto_modes_map = parse_map_list(args.proto_modes_map, default=["half","triangle","skewL","skewR"])
    ...
    # consensus maps (int/float)
    def parse_int_map(s, default=1):
        m = {p: default for p in PRIMS}
        if not s: return m
        for part in s.split(";"):
            if not part.strip(): continue
            name, val = part.split(":")
            m[name.strip()] = int(float(val))
        return m
    def parse_float_map(s, default=0.15):
        m = {p: default for p in PRIMS}
        if not s: return m
        for part in s.split(";"):
            if not part.strip(): continue
    ...
    consensus_k_map = parse_int_map(args.consensus_k_map, default=1)
    consensus_eps_map = parse_float_map(args.consensus_eps_map, default=0.15)
    ...
    for i in range(1, args.samples+1):
        ...
        keep, order, energies, peaks, margins = well_parser_step5(
            traces, rng, sigma=args.sigma, proto_width=args.proto_width, rank=args.rank,
            weights_map=weights_map, nperm=args.nperm, block=args.block,
            temp0=args.temp0, temp_min=args.temp_min, anneal=args.anneal, max_picks=args.max_picks,
            inhib_strength=args.inhib_strength, inhib_sigma=args.inhib_sigma, presence_floor=args.presence_floor,
            proto_modes_map=proto_modes_map, proto_phases=phases, proto_agg=args.proto_agg, proto_tau=args.proto_tau,
            raw_floor_map=raw_floor_map, residual_ceiling_map=residual_ceiling_map,
            consensus_k_map=consensus_k_map, consensus_eps_map=consensus_eps_map,
            ortho_flip_v=args.ortho_flip_v, pick_penalty=args.pick_penalty
        )
    ...
    print("[Step5] Summary:", {k: (round(v,3) if isinstance(v,float) else v) for k,v in summary.items()})
    if margins_all:
        print(f"[Step5] Margins — mean={np.mean(margins_all):.3f}, min={np.min(margins_all):.3f}, n={len(margins_all)}")
