...
# -*- coding: utf-8 -*-
"""
Stage 11 — Well Benchmark + Report (funnel-updated, PRIOR FIX)
---------------------------------------------------------------
This patch ensures that when --use_funnel_prior 1 is set, we actually
compute priors *before* parsing and pass them into the parser.

Changes vs. original:
- Added --calib_samples (default: min(400, max(120, samples)))
- Build calibration PCA cloud first → fit radial funnel → derive priors
- Then run the per-sample generation+parsing loop, using priors iff requested
- JSON notes whether priors were active

Outputs
  - manifold_pca3_mesh_warped.png
  - manifold_pca3_mesh_warped_fit.png
  - stage11_metrics.csv
...
def build_H_E_from_traces(args, n_samples: Optional[int] = None) -> Tuple[np.ndarray, np.ndarray]:
    """Return feature matrix H and scalar energy E per sample (for PCA viz)."""
    rng = _rng(args.seed + 10)  # offset to avoid reuse with parse loop
    H_rows, E_vals = [], []
    ns = int(n_samples if n_samples is not None else args.samples)
    for _ in range(ns):
...
def null_threshold(signal, proto, rng, K=40, q=0.95):
    K = int(max(1, K))                  # ← guard: at least 1 sample
    q = float(min(max(q, 0.0), 1.0))    # ← guard: keep q in [0,1]
    n = len(signal)
    vals = []
    for _ in range(K):
...
def main():
    ap = argparse.ArgumentParser(description="Stage 11 — warped manifold + funnel fit + stats (CSV/JSON) [PRIOR FIX]")
    # data
    ...
    # NEW: calibration batch size
    ap.add_argument("--calib_samples", type=int, default=None, help="Samples for PCA+funnel fit; default=min(400, max(120, samples))")
    args = ap.parse_args()

    # ----- CALIBRATION: build H/E and priors FIRST -----
    calib_n = int(args.calib_samples if args.calib_samples is not None else min(400, max(120, args.samples)))
    H_calib, E_calib = build_H_E_from_traces(args, n_samples=calib_n)
    base_params = WellParams(sigma_scale=args.sigma_scale, depth_scale=args.depth_scale, mix_z=args.mix_z)
    X3_warp, m, info = pca3_and_warp(H_calib, energy=E_calib, params=base_params)

    # Raw trisurf (warped cloud)
    fig, ax = plot_trisurf(X3_warp, energy=E_calib, title="Stage 11 — Warped Single Well (Calibration)")
    fig.savefig(args.out_plot, dpi=220); plt.close(fig)

    # Fit radial funnel on calibration
    ...
    def plot_surface(center, r_grid, z_prof, points, energy, n_theta, out_path, alpha=0.9):
        theta = np.linspace(0, 2*np.pi, n_theta)
        R, TH = np.meshgrid(r_grid, theta)
        X = center[0] + R * np.cos(TH)
        Y = center[1] + R * np.sin(TH)
    ...
    plot_surface(info["center"], r_grid, z_prof, X3_warp, E_calib, args.n_theta, out_fit)

    # prepare priors for optional coupling
    priors = priors_from_profile(r_grid, z_prof)

    # ----- per-sample generation + parsing (METRICS LOOP) -----
    rng = _rng(args.seed)
    rows = []
    ...
    summary = dict(
        ...
        plot_fitted=args.out_plot.replace(".png", "_fit.png"),
    ...
    print(f"[PLOT] FITTED:  {args.out_plot.replace('.png','_fit.png')}")
